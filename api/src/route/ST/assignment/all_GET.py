    ...
    try:
        ...
        for row in data:
            AllLab.append({
                ...
                "Lab": row[1], 
                "Name": row[2], 
                "Publish": datetime.strptime(str(row[3]), "%Y-%m-%d %H:%M:%S").strftime("%d/%m/%Y %H:%M") if row[3] else None, 
                "Due": datetime.strptime(str(row[4]), "%Y-%m-%d %H:%M:%S").strftime("%d/%m/%Y %H:%M") if row[4] else None,
                "Score": int(row[5]),
                "MaxScore": int(row[6]),
                "TurnIn": bool(row[7]),
                "Late": bool(row[8]),
