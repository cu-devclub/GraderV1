  ...
  const handleRemoveQuestion = (id) => {
    if (Question.length <= 1) {
      withReactContent(Swal).fire({
        title: "Cannot remove",
        text: "You must have at least 1 question.",
        icon: "warning"
      });
      return;
    }
  ...
  const validateAndFocus = () => {
    ...
    if (labName === '') {
      ...
      for (let i = 0; i < Question.length; i++) {
        const sourceInput = document.getElementById(`QSource${Question[i].id}`);
        const releaseInput = document.getElementById(`QRelease${Question[i].id}`);
        if (!sourceInput || !sourceInput.files || sourceInput.files.length === 0) {
          invalidTab = 'Questions';
          focusId = `QSource${Question[i].id}`;
          errorMessage = `Source file for Question ${i + 1} is required`;
          break;
        }
        if (!releaseInput || !releaseInput.files || releaseInput.files.length === 0) {
          invalidTab = 'Questions';
          focusId = `QRelease${Question[i].id}`;
          errorMessage = `Release file for Question ${i + 1} is required`;
          break;
        }
      }
    }
    ...
    if (invalidTab) {
      ...
      setTimeout(() => {
        ...
        if (el) {
          ...
          } else if (focusId.startsWith('QSource') || focusId.startsWith('QRelease')) {
             el.parentElement.style.borderColor = '#ef4444';
             el.parentElement.style.backgroundColor = '#fef2f2';
          } else {
    ...
    try{
      ...
      withReactContent(Swal).fire({
        ...
        if(ok.isConfirmed){
          ...
          if (inputlink && inputlink.files) {
            const addFiles = inputlink.files;
          ...
          for(let i = 0;i < Question.length;i++){
            const sourceInput = document.getElementById(`QSource${Question[i].id}`);
            const releaseInput = document.getElementById(`QRelease${Question[i].id}`);
            formData.append(`Source${i}`, sourceInput ? sourceInput.files[0] : undefined);
            formData.append(`Release${i}`, releaseInput ? releaseInput.files[0] : undefined);
          }
          ...
          formData.append("QNum", Question.length);
    ...
    <div>
        ...
        <div style={{ display: currentTab === 'Files' ? 'block' : 'none' }}>
          <div style={{ marginTop: '2rem' }}>
            ...
            <div className="upload-dropzone" style={{ position: 'relative', padding: '40px', borderRadius: '8px', textAlign: 'center', height: '250px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
              <input type="file" multiple id="inputlink" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: 0, cursor: 'pointer' }} onChange={(e) => { 
      ...
      <div className="responsive-bottom-bar" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, backgroundColor: 'white', borderTop: '1px solid #cbd5e1', padding: '16px 10em', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 1000, boxShadow: '0 -4px 6px -1px rgba(0, 0, 0, 0.05)' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          <button type="button" onClick={handleButtonClick} style={{ padding: '8px 32px', backgroundColor: '#e25595', border: 'none', color: 'white', borderRadius: '30px', fontWeight: '600', transition: 'all 0.2s' }}>Create</button>
        </div>
      </div>
